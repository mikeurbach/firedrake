...
status = recv(sock->tcp_sock, (&payload_len), 7, 0);

if(payload_len < PAYLOAD_EXT_16){

}
else if(payload_len == PAYLOAD_EXT_16){
	status = recv(sock->tcp_sock, (&ext_payload_len1), 16, 0);
}
else if(payload_len == PAYLOAD_EXT_64){
	status = recv(sock->tcp_sock, (&ext_payload_len2), 64, 0);
}
...
if(is_masked){
	status = recv(sock->tcp_sock, (&mask_key), 32, 0);
	//unmask the payload data
}

//Place Payload data in buffer and return status
 status = recv(sock->tcp_sock, buff, payload_len, 0);
	return (status);
}
